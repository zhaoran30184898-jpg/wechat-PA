...
class ClaudeClient(BaseAIClient):
    ...
    async def _create_client(self):
        ...
        if not self._validate_api_key():
            raise ValueError("Claude API Key 无效，请检查配置")
    ...
    async def rewrite_article(
        ...
        try:
            ...
            logger.info(f"正在使用 Claude 改写文章: {title[:50]}...")

            # 调用Claude API
            response = await self._generate_content_async(prompt)

            if not response or not response.content:
                raise Exception("Claude返回空响应")
    ...
    async def translate_text(self, text: str, target_language: str = "zh-CN") -> str:
        ...
        try:
            ...
            logger.debug(f"正在使用 Claude 翻译文本 ({len(text)} 字符)")
